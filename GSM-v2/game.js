...
function hud(){$("score-display").textContent=score;$("level-display").textContent=level;$("lives-display").textContent=lives;$("cret-display").textContent=Math.floor(crix);$("xp-display").textContent=xp;$("next-xp-display").textContent=xpNeed(level);const hpPct=Math.max(0,player?player.hp/player.max*100:100);$("player-hp-fill").style.width=hpPct+"%";$("hull-readout").textContent=Math.round(hpPct)+"%";$("ability-readout").textContent=abilityClock>0?abilityClock.toFixed(1)+"s":"READY";$("ability-bar-fill").style.width=Math.max(0,(1-abilityClock/8)*100)+"%"}
...
function shoot(){if(state!=="RUNNING"||fireClock>0)return;fireClock=weapon.delay;const s=$("bulletShoot");if(s){s.volume=.70;s.currentTime=0;s.play().catch(()=>{})}bullets.push({x:player.x,y:player.y-18,v:680,r:3,d:weapon.damage});if(weapon.dual){bullets.push({x:player.x-9,y:player.y-13,v:680,r:3,d:weapon.damage*.7},{x:player.x+9,y:player.y-13,v:680,r:3,d:weapon.damage*.7})}}
...
function ship(x,y){ctx.save();ctx.translate(x,y);ctx.globalAlpha=invuln>0&&Math.floor(invuln*15)%2===0?.3:1;ctx.shadowBlur=18;ctx.shadowColor="#00eaff";ctx.fillStyle="#00eaff";ctx.beginPath();ctx.moveTo(0,-25);ctx.lineTo(17,18);ctx.lineTo(6,12);ctx.lineTo(0,24);ctx.lineTo(-6,12);ctx.lineTo(-17,18);ctx.closePath();ctx.fill();ctx.fillStyle="#dffcff";ctx.beginPath();ctx.moveTo(0,-14);ctx.lineTo(6,8);ctx.lineTo(-6,8);ctx.closePath();ctx.fill();ctx.fillStyle="#39ddff";ctx.fillRect(-3,17,6,16);ctx.restore();ctx.globalAlpha=1;ctx.shadowBlur=0}
